...
import { Button, Row, Input, FormGroup, Col, Label } from 'reactstrap';
...
export default class MultiAddSales extends React.Component {
  ...
  getInitialList() {
    try {
      if (
        (this.props.existingSalesDetails != null ||
          this.props.existingSalesDetails != undefined) &&
        this.props.existingSalesDetails.length > 0
      ) {
        ...
        {this.state.inputList.map((x, i) => {
          return (
            <div>
              ...
              {x.upcomingSale != 'No Token Sales' &&
                !isEmpty(x.upcomingSale) && (
                  <div>
                    <FormGroup row>
                      <Label for="name" sm={inputnamewidth}>
                        Sale Status
